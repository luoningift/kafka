...
class JoinGroup extends Protocol
{
    ...
    public function encode(array $payloads = []): string
    {
        ...
        $header = $this->requestHeader('Easyswoole-kafka', self::JOIN_GROUP_REQUEST, self::JOIN_GROUP_REQUEST);
        $data   = self::encodeString($payloads['group_id'], self::PACK_INT16); //group_id STRING
        $data  .= self::pack(self::BIT_B32, (string) $payloads['session_timeout']);// INT32

        if ($this->getApiVersion(self::JOIN_GROUP_REQUEST) === self::API_VERSION1) {
            $data .= self::pack(self::BIT_B32, (string) $payloads['rebalance_timeout']);// INT32
        }

        $data .= self::encodeString($payloads['member_id'], self::PACK_INT16);// STRING
        $data .= self::encodeString($payloads['protocol_type'], self::PACK_INT16);// STRING
    ...
    protected function encodeGroupProtocol(array $values): string
    {
        ...
        $protocolName = self::encodeString($values['protocol_name'], self::PACK_INT16);// STRING
