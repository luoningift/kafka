...
class Fetch extends Protocol
{
    ...
    public function encode(array $payloads = []): string
    {
        ...
        $header = $this->requestHeader('Easyswoole-kafka', self::FETCH_REQUEST, self::FETCH_REQUEST);
    ...
    public function decode(string $data): array
    {
        ...
        if ($version !== self::API_VERSION0) {
            $throttleTime = self::unpack(self::BIT_B32, substr($data, $offset, 4));// throttle_time_ms int32
            $offset      += 4;
        }
    ...
    {
        $offset    = 0;
        $topicInfo = $this->decodeString(substr($data, $offset), self::BIT_B16);//topic string
    ...
    {
        $offset              = 0;
        $partitionId         = self::unpack(self::BIT_B32, substr($data, $offset, 4));// partition int32
        $offset             += 4;
        $errorCode           = self::unpack(self::BIT_B16_SIGNED, substr($data, $offset, 2));// error_code int16
        $offset             += 2;
        $highwaterMarkOffset = self::unpack(self::BIT_B64, substr($data, $offset, 8));//high_watermark int64
        $offset             += 8;
    ...
    protected function decodeMessage(string $data, int $messageSize): ?array
    {
        ...
        try { // try unpack message format v1, falling back to v0 if it fails
            ...
            $valueRet = $this->decodeString(
                (string) substr($data, $offset),
                self::BIT_B32,
                $attr & Produce::COMPRESSION_CODEC_MASK
            );
        ...
        } catch (Exception\Exception $e) { // try unpack message format v0
            $offset    = $backOffset;
            $timestamp = 0;
            $keyRet    = $this->decodeString(substr($data, $offset), self::BIT_B32);
            $offset   += $keyRet['length'];
